...
import { addSaleItem, incSaleitemQty, saveTicketDetail, updateCheckoutTotals, updateServedByAssociate } from '../../store/ticketstore/ticket.action';
import { getCheckoutItemsCount, getCheckoutItemsSelector, getTranIdTicketNumber } from '../../store/ticketstore/ticket.selector';
...
export class SaleItemComponent implements OnInit {
  ...
  saleItemsInTicket: SalesTransactionCheckoutItem[] | null = [];
  ...
  ngOnInit(): void {
    ...
    this._store.select(getCheckoutItemsSelector).subscribe(data => {
      // Handle checkout items data if needed
      this.saleItemsInTicket = data;
    });
  }

  public salesItemClick(event: Event, itemId: number): void {

    let ltVendorLogonData = this._logonDataSvc.getLTVendorLogonData();

    const saleCheckoutItem = this.getSaleCheckOutItem(
      this.saleItemList.filter(itm => itm.salesItemID == itemId)[0]);

    const existingItem = this.saleItemsInTicket?.find(itm => itm.salesItemUID == saleCheckoutItem.salesItemUID);
    if (this._logonDataSvc.getAllowTips() == false) {

      if (existingItem) {
        this._store.dispatch(incSaleitemQty({ saleItemId: existingItem.salesItemUID, tktDtlId: existingItem.ticketDetailId, defCurrSymbl: this.dcCurrSymbl, dailyExchRateObj: this._logonDataSvc.getDailyExchRate() }));
      }
      else {
      ...
      if (this.transactionId > 0) {
        ...
        //get the existing item from store afresh with updated quantity and other values
        const existingItem = this.saleItemsInTicket?.find(itm => itm.salesItemUID == saleCheckoutItem.salesItemUID);
        if (existingItem) {
          const individualUid = Number(ltVendorLogonData.individualUID || 0);
    ...
    saleCheckoutItem.srvdByAssociateText = ltVendorLogonData.associateName;
    this._store.dispatch(addSaleItem({ saleItem: saleCheckoutItem, defCurrSymbl: this.dcCurrSymbl, dailyExchRateObj: this._logonDataSvc.getDailyExchRate() }));
